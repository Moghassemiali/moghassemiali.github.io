---
layout: archive
title: " 🧑🏻‍🏫 Teaching"
permalink: /teaching/
author_profile: true
---

------

## <span style="border-left:10px solid #003f87; padding-left:12px; font-size:1.3rem; font-weight:800;">University of Wisconsin-Milwaukee (UWM)</span>

### <span style="border-left:6px solid #1E90FF; padding-left:10px;">Spring/Fall '26</span>

- ELECENG 362 -- Electromechanical Energy Conversion -- Undergraduate -- 4 Credits (3 lect. and 1 lab.)
    <details style="background:#fffaf0; padding:12px; border-radius:8px; border:1px solid #d9e9ff; margin-top:10px; transition:0.3s;">
    <summary><strong style="color:#FF8C00;">Group Photo</strong></summary>
    <img src="{{ '/images/gallery/classes/ELECENG362_Spring2026.JPG' | relative_url }}" 
      style="width:100%; margin-top:10px;" alt="Spring 2026 - Lubar Hall N110">
      <p style="text-align:center; margin-top:8px; font-size:0.95rem; color:#555;"> Spring 2026 - Lubar Hall N110
      </p>

## <span style="border-left:10px solid #003f87; padding-left:12px; font-size:1.3rem; font-weight:800;">University of Applied Science and Technology</span>

### <span style="border-left:6px solid #1E90FF; padding-left:10px;">Spring '21</span>

- Industrial Electronics -- Undergraduate -- 3 Credits
- Power Systems Analysis -- Undergraduate -- 3 Credits
- Protections and Relays -- Undergraduate -- 3 Credits
- Electric Machines -- Undergraduate -- 3 Credits

## <span style="border-left:10px solid #003f87; padding-left:12px; font-size:1.3rem; font-weight:800;">Islamic Azad University, South Tehran</span>

### <span style="border-left:6px solid #1E90FF; padding-left:10px;">Fall '12, Spring/Fall '13, Spring/Fall '14, Spring/Fall '15, Spring '16</span>

- MATLAB/Simulink -- Undergraduate/Graduate -- 3 Credits

### <span style="border-left:6px solid #1E90FF; padding-left:10px;">Fall '10, Spring/Fall '11, Spring/Fall '12, Spring/Fall '13, Spring/Fall '14, Spring '15</span>

- Teaching Assistant -- Electric Circuits I -- Undergraduate -- 3 Credits
- Teaching Assistant -- Electric Circuits II - Undergraduate -- 3 Credits
- Teaching Assistant -- Power Electronics -- Graduate -- 3 Credits
- Teaching Assistant -- Advanced Power Systems Operation -- Graduate -- 3 Credits
